    ...
    getChartDataByUserId: async (userId) => {
        try {
            ...
            if (monthlyTransactions.length > 0) {
                ...
                const budgets = aggregatedMonthlyTransactions.map(function(transaction) {
                ...
                chartElements = budgets.map(element => {
                    ...
                    if (expenses.map(expense => expense.period).includes(element.period)) {
                        element_expenses = expenses.find(expense => expense.period === element.period)!.amount;
                    } else {
                        element_expenses = 0;
                    }
    
                    let element_deposits: number;
                    if (deposits.map(deposit => deposit.period).includes(element.period)) {
                        element_deposits = deposits.find(deposit => deposit.period === element.period)!.amount;
                    } else {
                        element_deposits = 0;
                    }
